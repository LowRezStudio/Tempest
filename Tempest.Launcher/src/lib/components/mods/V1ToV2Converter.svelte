<script lang="ts">
	...
	type SourceEntry = {
		label: string;
		files: FileEntry[];
		manifest?: { name: string; id: string; version: string; authors: { name: string; link: string }[] };
		readme?: string;
	};
	...
	async function pickFiles() {
		const result = await openDialog({
			multiple: true,
			filters: [
				{ name: "Mod Files", extensions: ["tempest", "upk", "pck", "ini"] },
			],
		});
		if (!result) return;
	...
	async function processFile(filePath: string) {
		...
		try {
			if (ext === "tempest") {
				...
				addToast({ title: m.toast_unsupported_file_title(), message: m.toast_unsupported_file_message(), tone: "error" });
			}
		} finally {
			importingLabel = null;
		}
		...
		try {
			...
			if (manifest && !converter.modName) {
				...
				if (manifest.authors?.length) {
					converter.authors = manifest.authors.map((a) => ({ name: a.name, link: a.link || "" }));
				}
			}
	...
	async function processLooseFile(filePath: string) {
		...
		converter.sources = [...converter.sources, { label: fileName, files: [{ path: `${targetDirFor(fileName)}/${fileName}`, data }] }];
	}
	...
	function updateTargetPath(sourceIdx: number, fileIdx: number, newPath: string) {
		...
		updated[sourceIdx] = {
			...updated[sourceIdx],
			files: updated[sourceIdx].files.map((f, i) => (i === fileIdx ? { ...f, path: newPath } : f)),
		};
	...
	async function cook() {
		...
		try {
			...
			converter.cookedZip = await zip.generateAsync({ type: "uint8array", compression: "DEFLATE" });
		} catch (error) {
	...
	async function download() {
		...
		try {
			...
			addToast({ title: m.converter_toast_saved_title(), message: m.converter_toast_saved_message({ path: savePath }), tone: "success" });
		} catch (error) {
	...
	async function installToInstance(inst: Instance) {
		...
		try {
			installingToastId = addToast({ title: m.toast_mod_installing_title(), message: m.toast_mod_installing_message({ name: modFileName }), tone: "info", duration: 0 });
			...
			if (result.Success) {
				addToast({ title: m.toast_mod_installed_title(), message: m.converter_toast_installed_message({ name: converter.modName, instance: inst.label }), tone: "success" });
				queryClient.invalidateQueries({ queryKey: ["mods", inst.path] });
			} else {
				addToast({ title: m.toast_installation_failed_title(), message: result.Message || m.common_unknown(), tone: "error" });
			}
			try { await import("@tauri-apps/plugin-fs").then(m => m.remove(tmpFile)); } catch {}
		} catch (error) {
			if (installingToastId) removeToast(installingToastId);
	...
	>
		...
		>
			<div class="card-body items-center gap-3">
				<div class="w-16 h-16 rounded-xl bg-accent/10 flex items-center justify-center text-accent">
					<FlaskConical size={32} />
				</div>
				<h2 class="card-title justify-center">{m.converter_title()}</h2>
				<p class="text-sm opacity-70">
					{m.converter_drop_hint({ tempest: ".tempest", upk: ".upk", pck: ".pck", ini: ".ini" })}
				</p>
			</div>
		</div>
	</div>
		...
		<div class="flex-1 overflow-y-auto">
			<div class="px-4 py-6">
				...
				{#if converter.sources.length === 0}
					<div class="border-2 border-dashed border-base-300 rounded-box p-12 text-center mb-4">
						<div class="flex flex-col items-center gap-2 text-base-content/50">
							<FileUp size={32} />
							<p class="text-sm">{m.converter_drop_hint({ tempest: ".tempest", upk: ".upk", pck: ".pck", ini: ".ini" })}</p>
						...
						{#each converter.sources as source, sIdx (sIdx)}
							<div class="bg-base-200 rounded-lg p-4">
								<div class="flex items-center justify-between mb-2">
									<span class="text-sm font-mono font-semibold truncate">{source.label}</span>
									<button class="btn btn-ghost btn-sm btn-square text-error" onclick={() => removeSource(sIdx)}>
										<Trash2 size={14} />
									</button>
								</div>
								<div class="space-y-1">
									{#each source.files as file, fIdx}
										<div class="pl-2">
											<input
												...
												value={file.path}
												oninput={(e) => updateTargetPath(sIdx, fIdx, e.currentTarget.value)}
											/>
										</div>
									{/each}
								</div>
				...
				{:else}
					<p class="text-xs text-base-content/50 mb-6">
						{totalFiles} {m.downloads_files({ count: totalFiles })} across {converter.sources.length} {m.converter_sources({ count: converter.sources.length })}
					</p>
				{/if}
				...
				<div class="flex flex-col gap-4">
					<div class="flex flex-col gap-1">
						...
						<input id="conv-name" type="text" class="input input-bordered w-full" bind:value={converter.modName} placeholder={m.converter_mod_name_placeholder()} />
					</div>

					<div class="flex flex-col gap-1 w-32">
						<span class="text-sm font-semibold">{m.common_version()}</span>
						<input id="conv-version" type="text" class="input input-bordered w-full" bind:value={converter.modVersion} placeholder={m.converter_version_placeholder()} />
					</div>
				</div>
							...
							<div class="bg-base-200 rounded-lg p-4">
								<div class="flex flex-col gap-2">
									<input
										...
										value={author.name}
										oninput={(e) => updateAuthor(i, "name", e.currentTarget.value)}
									/>
									<div class="flex items-center gap-2">
										<input
											...
											value={author.link}
											oninput={(e) => updateAuthor(i, "link", e.currentTarget.value)}
										/>
										<button class="btn btn-ghost btn-sm btn-square text-error shrink-0" onclick={() => removeAuthor(i)}>
											<X size={14} />
										</button>
									</div>
								</div>
				...
				<div class="form-control mb-6">
					<span class="text-sm font-semibold mb-2 block">{m.converter_readme_label()}</span>
					<textarea
						id="conv-readme"
						class="textarea textarea-bordered h-48 w-full"
						bind:value={converter.readmeContent}
						placeholder={m.converter_readme_placeholder()}
					></textarea>
				</div>

				<div class="flex items-center justify-between">
					<button class="btn btn-accent gap-2" disabled={!canCook || cooking} onclick={cook}>
						{#if cooking}
							<span class="loading loading-spinner loading-xs"></span>
							{m.converter_cooking()}
						{:else}
...
<InstanceSelectModal
	...
	onselect={(inst) => { void installToInstance(inst); }}
	oncancel={() => { showInstanceSelect = false; }}
/>
