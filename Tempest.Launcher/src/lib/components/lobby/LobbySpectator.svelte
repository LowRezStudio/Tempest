<script lang="ts">
	...
	import { isSpectator, players, playerId, spectators } from "$lib/lobby/stores.svelte";
...
<div class="absolute bottom-8 right-8 z-20 flex flex-col items-end gap-2">
	{#if spectators.value.length > 0}
		<div class="bg-base-200/95 backdrop-blur-xs rounded-lg shadow-xl px-3 py-2 max-w-52">
	...
	{/if}
	{#if ownPlayer}
		<button class="btn btn-sm shadow-none" onclick={handleToggle}>
			{#if isSpectator.value}
				<EyeOff size={16} />
				{m.lobby_join_team()}
			{:else}
