<script lang="ts">
	...
	let isLoading = $derived(
		activeTab === "internet"
			? (serversQuery.isLoading ?? false)
			: (lanQuery.isLoading ?? false),
	);
	let isFetching = $derived(
		activeTab === "internet"
			? (serversQuery.isFetching ?? false)
			: (lanQuery.isFetching ?? false),
	);
			...
			<div class="px-4 py-6">
				{#if isLoading}
					<div class="flex h-64 flex-col items-center justify-center gap-4">
						...
						></span>
						<p class="text-base-content/50 text-lg">{m.serverlist_loading()}</p>
					</div>
				{:else if error}
					<div role="alert" class="alert alert-error">
						...
						<span
							>{formatQueryError(
								activeTab === "internet" ? serversQuery.error : lanQuery.error,
							)}</span
						>
					</div>
				{:else if servers.length === 0}
					<div class="flex h-64 flex-col items-center justify-center gap-4">
						...
						<p class="text-base-content/50 text-lg">{m.serverlist_no_servers()}</p>
